...
class GolangLoader(loader.TestLoader):
    ...
    def discover(self, url, which_tests=loader.DEFAULT):
        ...
        if go_root is not None:
            package_paths.append(os.path.join(os.path.expanduser(go_root),
                                              'src'))
        if go_path is not None:
            package_paths.append(os.path.join(os.path.expanduser(go_path),
                                              'src'))

        if ':' in url:
            url, _tests_filter = url.split(':', 1)
            if '/' in _tests_filter:
                _tests_filter, subtest = _tests_filter.split('/', 1)
        ...
        if not test_files:
            if which_tests == loader.ALL:
                msg = ('No package matching this reference.')
