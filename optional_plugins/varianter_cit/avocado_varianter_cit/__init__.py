...
class VarianterCit(Varianter):
    ...
    def initialize(self, args):
        ...
        cit = Cit(parameters, constraints, order)
...
class Cit(object):
    ...
    def __init__(self, parameters, constraints, order):
        self.parameters = parameters
        self.data = [len(param[1]) for param in self.parameters]
    ...
    def combine(self):
        ...
        result = [[self.parameters[i][1][combination[i]] for i in range(len(combination))]for combination in final_list]
        combinations = []
    ...
    def create_interaction_hash_table(self):
        for c in itertools.combinations(range(len(self.data)), self.order):
            self.hash_table[':'.join(map(str, c))] = self.get_interaction(c)
    ...
    def get_weight(self, test_case, remove_list):
        ...
        for i in self.hash_table:
            ...
            array = i.split(':')
            for j in array:
                interaction_array.append(test_case[int(j)])
            interaction = ''.join(map(str, interaction_array))
            try:
                value = self.hash_table[i][interaction]
                weight += 1
                remove_list[i] = value
            except KeyError:
                continue
    ...
    def neighborhood_search(self, test_case, width, max_width, remove_list):
        ...
        for i in range(len(test_case)):
            # neighborhood +1
            if (neighborhood[i] + 1) == self.data[i]:
                neighborhood[i] = 0
            else:
                neighborhood[i] += 1
