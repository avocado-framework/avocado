...
def combination_row_equals(row_1, row_2):
    return (row_1.covered_more_than_ones == row_2.covered_more_than_ones
        and row_1.uncovered == row_2.uncovered
...
class MatrixInitialization(unittest.TestCase):
    ...
    def test_combination_matrix_initialization(self):
        ...
        self.assertEqual(matrix.total_uncovered, excepted_uncovered,
            "Total uncovered number is wrong.")
        self.assertEqual(matrix.total_covered_more_than_ones,
            excepted_covered_more_than_ones,
            "Total uovered_more_than_ones number is wrong.")
        self.assertEqual(len(matrix.hash_table), excepted_row_size,
            "Matrix has wrong row size")
        self.assertEqual(len(matrix.uncovered_rows), excepted_row_size,
            "Matrix has wrong uncovered row size")
        for key, value in matrix.hash_table.items():
            with self.subTest(combination=key):
                self.assertTrue(combination_row_equals(
                    value, excepted_hash_table[key]))
...
class CombinationMatrixTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.excepted_hash_table = {(0, 1): CombinationRow(
                                        self.data, self.t_value, (0, 1)),
                                    (0, 2): CombinationRow(
                                        self.data, self.t_value, (0, 2)),
                                    (0, 3): CombinationRow(
                                        self.data, self.t_value, (0, 3)),
                                    (1, 2): CombinationRow(
                                        self.data, self.t_value, (1, 2)),
                                    (1, 3): CombinationRow(
                                        self.data, self.t_value, (1, 3)),
                                    (2, 3): CombinationRow(
                                        self.data, self.t_value, (2, 3))}
    ...
    def test_cover_solution_row(self):
        ...
        self.assertEqual(excepted_uncovered, self.matrix.total_uncovered,
            "Total uncovered number is wrong.")
        self.assertEqual(excepted_covered_more_than_ones,
            self.matrix.total_covered_more_than_ones,
            "Total uovered_more_than_ones number is wrong.")
        self.assertEqual(excepted_uncovered_row_size,
            len(self.matrix.uncovered_rows),
            "Matrix has wrong uncovered row size")
        for key, value in self.matrix.hash_table.items():
            with self.subTest(combination=key):
                self.assertTrue(combination_row_equals(value,
                    self.excepted_hash_table[key]))
