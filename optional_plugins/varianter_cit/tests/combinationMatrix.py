...
def combination_row_equals(row_1, row_2):
    return (
        row_1.covered_more_than_ones == row_2.covered_more_than_ones
...
class MatrixInitialization(unittest.TestCase):
    def test_combination_matrix_initialization(self):
        ...
        excepted_row_size = 6
        excepted_hash_table = {
            (0, 1): CombinationRow(data, t_value, (0, 1)),
        ...
        self.assertEqual(
            matrix.total_uncovered,
            excepted_uncovered,
            "Total uncovered number is wrong.",
        )
        self.assertEqual(
            matrix.total_covered_more_than_ones,
            excepted_covered_more_than_ones,
            "Total uovered_more_than_ones number is wrong.",
        )
        self.assertEqual(
            len(matrix.hash_table), excepted_row_size, "Matrix has wrong row size"
        )
        self.assertEqual(
...
class CombinationMatrixTest(unittest.TestCase):
    def setUp(self):
        ...
        self.excepted_hash_table = {
            (0, 1): CombinationRow(self.data, self.t_value, (0, 1)),
    ...
    def test_cover_solution_row(self):
        ...
        for key, value in self.matrix.hash_table.items():
            with self.subTest(combination=key):
                self.assertTrue(
                    combination_row_equals(value, self.excepted_hash_table[key])
                )
