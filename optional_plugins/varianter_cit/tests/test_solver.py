...
class SolverTest(unittest.TestCase):
    ...
    def test_compute_constraints_without_secret_constraint(self):
        """
        Test that, function shouldn't change constraints
        if there isn't any secret constraint
    ...
    def test_compute_constraints_new_constraint_with_more_than_one_value_for_one_parameter(self):
        """
        Test that, function shouldn't change constraints.
        It founds new constraint, but the constraint has one parameter
        with two values. This means that it isn't secreted constraint.
        """
        parameters = [3, 3, 3]
    ...
    def test_compute_constraints_detect_invalid_constraints(self):
        """
        Test that, function should raise an exception,
        if there is invalid_constraints
        """
        parameters = [3, 3, 3]
    ...
    def test_compute_constraints_with_one_secrete_constraint(self):
        ...
        """
        parameters = [3, 3, 3, 3]
        constraints = {((0, 0), (2, 0)), ((0, 1), (1, 1), (2, 0)), ((0, 2),
            (3, 2))}
        solver = Solver([], [])
        solver.data = parameters
        solver.constraints = constraints
        solver.read_constraints()
        solver.compute_constraints()
        expectation = {((0, 0), (2, 0)), ((0, 1), (1, 1), (2, 0)), ((0, 2),
            (3, 2)), ((1, 1), (2, 0), (3, 2))}
    ...
    def test_solver_without_secrete_constraints(self):
        """
        Test that, solver didn't change constraints if
        there isn't any secret constraint
        """
        parameters = [3, 3, 3]
        constraints = {((0, 0), (2, 0)), ((0, 1), (1, 1)), ((1, 0), (2, 0))}
    ...
    def test_solver_constraints_with_simplification_and_secrete(self):
        """
        Test that, Minimum forbidden tuple algorithm can find
        and simplify constraints
        """
        parameters = [3, 3, 3, 3]
