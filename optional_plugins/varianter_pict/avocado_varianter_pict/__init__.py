...
class VarianterPict(Varianter):
    ...
    def initialize(self, args):
        ...
        if not os.access(pict_parameter_file, os.R_OK):
            LOG_UI.error("pict parameter file '%s' could not be found or is "
                         "not readable", pict_parameter_file)
            error = True

        pict_binary = getattr(args, "pict_binary", None)
        if pict_binary is None:
            LOG_UI.error("pict binary '%s' could not be found, please set it "
                         "with --pict-binary or put it in your PATH",
                         pict_binary)
            error = True
        elif not os.access(pict_binary, os.R_OK | os.X_OK):
