...
develop:
	$(PYTHON) -m pip install -e . $(PYTHON_DEVELOP_ARGS)
...
%.1: %.rst
	...
	elif command -v rst2man.py >/dev/null 2>&1; then \
		rst2man.py man/avocado.rst man/avocado.1; \
	else \
...
.PHONY: pip install clean uninstall requirements-dev smokecheck check develop develop-core develop-external develop-plugins develop-plugin variables man
