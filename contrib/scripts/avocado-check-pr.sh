...
# Parse arguments
while getopts "ivu:t:r:dhD" opt; do
    case $opt in
        i)
            INTERACTIVE=1
            ;;
...
GIT_PR_COMMITS=$(git cherry $GIT_REMOTE/$BRANCH | sed -n 's/+ \(.*\)/\1/p')
# All commits between first-1 commit and HEAD
GIT_RANGE=$(git rev-list --reverse $(echo "$GIT_PR_COMMITS" | head -n 1)~1..HEAD)
[ "$?" -ne 0 ] && { echo "Error parsing git range using $GIT_REMOTE/$BRANCH"; exit -1; }

echo "Checking $BRANCH..$GIT_REMOTE/$BRANCH ($(echo "$GIT_PR_COMMITS" | wc -l | xargs)/$(echo "$GIT_RANGE" | wc -l | xargs))"
if [ "$VERBOSE" ]; then
    echo "PR commits:"
    echo "$GIT_PR_COMMITS"
    echo
