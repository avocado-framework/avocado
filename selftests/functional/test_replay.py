...
class ReplayTests(unittest.TestCase):
    ...
    def run_and_check(self, cmd_line, expected_rc, path=None):
        if path is not None:
            os.chdir(path)
        else:
            os.chdir(basedir)
    ...
    def test_run_replay_muxfiles(self):
        ...
        expected_rc = exit_codes.AVOCADO_ALL_OK
        result = self.run_and_check(cmd_line, expected_rc)
        msg = "Overriding the replay multiplex with --multiplex-files."
        self.assertIn(msg, result.stdout)

    def test_run_replay_muxfiles_chdir(self):
        path = os.path.join(basedir, 'scripts')
        cmd_line = ('./avocado run --replay %s --multiplex '
                    '../examples/tests/synctest.py.data/synctest.yaml '
                    '--job-results-dir %s --replay-data-dir %s '
                    '--sysinfo=off' % (self.jobid, self.tmpdir, self.jobdir))
        expected_rc = exit_codes.AVOCADO_ALL_OK
        result = self.run_and_check(cmd_line, expected_rc, path)
        msg = "Overriding the replay multiplex with --multiplex-files."
