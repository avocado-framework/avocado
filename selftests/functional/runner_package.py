...
class RunnableRun(unittest.TestCase):
    def test_no_kwargs(self):
        res = process.run(f"{RUNNER} runnable-run -k package", ignore_status=True)
    ...
    def test_action_check_alone(self):
        action = "action=check"
        res = process.run(
            f"{RUNNER} runnable-run -k package {action}", ignore_status=True
        )
        self.assertIn(b"'status': 'started'", res.stdout)
        self.assertIn(b"'status': 'finished'", res.stdout)
        self.assertIn(b"'time': ", res.stdout)
...
class TaskRun(unittest.TestCase):
    def test_no_kwargs(self):
        res = process.run(
            f"{RUNNER} task-run -i XXXreq-pacXXX -k package", ignore_status=True
        )
        self.assertIn(b"'status': 'finished'", res.stdout)
