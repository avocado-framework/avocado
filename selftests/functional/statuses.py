...
#          ...}
EXPECTED_RESULTS = {
    "SkipSetup.test": ("SKIP", ["TestSkipError: from setUp()"]),
    "SkipTest.test": ("SKIP", ["TestSkipError: from test()"]),
    "SkipTeardown.test": (
        "ERROR",
        [
            "setup pre",
    ...
    "CancelSetup.test": (
        "CANCEL",
        ["setup pre", "teardown pre", "teardown status: CANCEL", "teardown post"],
    ),
    "CancelTest.test": (
        "CANCEL",
        [
    ...
    "FailSetup.test": (
        "ERROR",
        ["setup pre", "teardown pre", "teardown status: ERROR", "teardown post"],
    ),
    "FailTest.test": (
        "FAIL",
        [
...
class TestStatuses(TestCaseTmpDir):
    def setUp(self):
        ...
        test_file = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__), os.path.pardir, ".data", "test_statuses.py"
            )
        )
    ...
    def test(self):
        ...
        self.assertEqual(
            missing_msg,
            "",
            (f"Expected results not found for class/method: " f"{missing_msg}"),
        )
