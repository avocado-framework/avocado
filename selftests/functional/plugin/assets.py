...
class AssetsFetchSuccess(TestCaseTmpDir):
    ...
    def test_asset_fetch_success(self):
        ...
        cmd_line = (
            f"{AVOCADO} --config {self.config_file.name} "
    ...
    def test_asset_register_by_name_fail(self):
        ...
        self.assertIn("Failed to fetch", result.stderr_text)
    ...
    def test_asset_register_by_name_success(self):
        ...
        self.assertIn("Now you can reference it by name hosts", result.stdout_text)
    ...
    def test_asset_purge(self):
        ...
        cmd_line = (
            f"{AVOCADO} --config {config} assets purge " f"--by-size-filter '==1'"
        )
...
class AssetsPlugin(unittest.TestCase):
    ...
    def test_asset_fetch(self):
        ...
        """
        expected_stderr = (
            "avocado assets fetch: error: the following"
    ...
    def test_asset_fetch_unsupported_file(self):
        ...
        test_content = TEST_TEMPLATE.format(content=fetch_content)
        test_file = tempfile.NamedTemporaryFile(
            suffix=".c", dir=self.base_dir.name, delete=False
        )
        test_file.write(test_content.encode())
        test_file.close()

        expected_stderr = f"No such file or file not supported: " f"{test_file.name}\n"
        expected_rc = exit_codes.AVOCADO_FAIL
