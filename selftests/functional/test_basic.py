...
class RunnerOperationTest(unittest.TestCase):
    ...
    def test_dry_run(self):
        ...
        assert '/tmp' in debuglog   # Use tmp dir, not default location
        assert result['job_id'] == u'0' * 40
        # Check if all tests were skipped
        assert result['skip'] == 3
        for i in xrange(3):
            test = result['tests'][i]
            assert test['fail_reason'] == u'Test skipped due to --dry-run'
        # Check if all params are listed
        # The "/:bar ==> 2 is in the tree, but not in any leave so inaccessible
        # from test.
        for line in ("/:foo ==> 1", "/:baz ==> 3", "/foo:foo ==> a",
                     "/foo:bar ==> b", "/foo:baz ==> c", "/bar:bar ==> bar"):
            assert log.count(line) == 3
