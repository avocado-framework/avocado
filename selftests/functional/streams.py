...
class StreamsTest(TestCaseTmpDir):
    ...
    def test_default_streams(self):
        ...
        self.assertEqual(result.exit_status, exit_codes.AVOCADO_ALL_OK)
        self.assertIn(b"Avocados have been harvested.", result.stdout)
        with open(job_log_path, "rb") as log:
            self.assertIn(b"Seeds have been palanted.", log.read())
        with open(test_log_path, "rb") as log:
            self.assertIn(b"waiting for Avocados to grow", log.read())
