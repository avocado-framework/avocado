...
class VMImageTest(unittest.TestCase):
...
class VMImageHtmlParserTest(unittest.TestCase):
    def setUp(self):
        pattern = '^[0-9]+/$'
...
class ImageProviderBaseTest(unittest.TestCase):
    def _get_html_with_versions(self, versions):
    ...
    def test_get_version(self, urlopen_mock):
        html_fixture = self._get_html_with_versions([10, 11, 12])
    ...
    def test_get_version_with_float_versions(self, urlopen_mock):
        html_fixture = self._get_html_with_versions([10.1, 10.3, 10.2])
        urlread_mocked = mock.Mock(return_value=html_fixture)
        urlopen_mock.return_value = mock.Mock(read=urlread_mocked)
    ...
    def test_get_version_with_string_versions(self, urlopen_mock):
        html_fixture = self._get_html_with_versions(['abc', 'abcd', 'abcde'])
        urlread_mocked = mock.Mock(return_value=html_fixture)
        urlopen_mock.return_value = mock.Mock(read=urlread_mocked)
    ...
    def test_get_version_versions_not_found(self, urlopen_mock):
        html_fixture = self._get_html_with_versions([])
        urlread_mocked = mock.Mock(return_value=html_fixture)
        urlopen_mock.return_value = mock.Mock(read=urlread_mocked)
        base_image = vmimage.ImageProviderBase(version='[0-9]+', build=None, arch=None)
