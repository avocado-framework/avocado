...
class JobTest(unittest.TestCase):
    ...
    def test_job_empty_suite(self):
        ...
        none_attrs = ("logdir", "replay_sourcejob", "result", "sysinfo",
                      "test_runner", "test_suite", "tmpdir")
        for attr in none_attrs:
            self.assertIsNone(getattr(self.job, attr), "job.%s is not None "
                              "(%s)" % (attr, getattr(self.job, attr)))
        ...
        self.job.setup()
        self.assertTrue(self.job.logdir)
        self.assertTrue(self.job.logfile)
        self.assertTrue(self.job.result)
        self.assertTrue(self.job._Job__remove_tmpdir)
        self.assertTrue(self.job.tmpdir)
