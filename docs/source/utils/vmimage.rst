...
Get the image object, which will contains the path for the base image
and for the external snapshot created out of the base image::
...
If you need to create your own Image Provider, you can extend the
``vmimage.IMAGE_PROVIDERS`` list, including your provider class.
For instance, using the ``vmimage`` utility in an Avocado test, we
could add our own provider with::
