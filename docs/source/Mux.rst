...
The ``Varianter`` object is defined in ``avocado/core/multiplexer.py``, is always
instantiated in ``avocado.core.parser.py`` and always available in
``args.xxx_variants``. The basic workflow is:

1. Initialize ``Variants`` in ``args.xxx_variants``
...
term ``Multiplexation`` or ``Variants`` to generate multiple variants of the same
