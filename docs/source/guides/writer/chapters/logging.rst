...
--store-logging-stream and external logs
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If your tests use some external library with its own logging namespace, those logs won't be
stored by default, but you can enable it with  ``--store-logging-stream``. Let's see
an example with Matplotlib external library:

.. literalinclude:: ../../../../../examples/tests/external_logging_stream.py

If you run this test without ``--store-logging-stream`` the Matplotlib logs won't be part of
avocado logs, because this library uses its own logging namespace ``matplotlib``.
You can enable them with ``--store-logging-stream=matplotlib``. It will add Matplotlib logs
into the standard avocado logs and it will create separated file ``matplotlib.log`` with only
logs from Matplotlib::

     $ avocado run --store-logging-stream=matplotlib examples/tests/external_logging_stream.py
