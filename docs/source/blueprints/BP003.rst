...
come from the ``TRIAGING`` phase and never gone through the
``STARTED`` phase.  Alternatively, it may been in the ``STARTED``
phase and finished without any errors.
...
Task #1 is not ready to be executed, so it's moved to ``FINISHED`` and it's
reason is recorded.
...
As a reminder, Task #1 has not passed triaging and is ``FINISHED``.
...
``STARTED``) is also exemplified.  As a general rule, all tasks are
attempted to be moved further into their life-cycle and a number
...
:class:`asyncio.Lock`.  It may aslo use individual :class:`Tasks
<asyncio.Task>` for each work in each phase.

.. literalinclude:: ../../../examples/task-lifecycle.py
