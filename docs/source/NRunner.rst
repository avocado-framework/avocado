...
runnable, and will be able to able to act upon the specific
information that runnable kind provides.

The following pseudo-code may help to illustrate that::

  runnable_instance = create_runnable('exec', path='/bin/sleep')
  if runnable.kind == 'exec':
     runner = create_runner_exec(runnable)
