...
- Create a ``remote``, pointing to the upstream::
    ...
    $ git remote add upstream git@github.com:avocado-framework/avocado-misc-tests.git
...
- Write a good commit message, pointing motivation, issues that you're
  ...
  message: motivation, approach and effects::
    ...
    header          <- Limited to 50 characters. No period.
                    <- Blank line
...
going to change the code based on some review and, if not, why. New versions
of your code should not be force-updated (unless explicitly requested by
the code reviewer). Instead, you should:
...
- Rebase your local branch on top of upstream master::
    ...
    $ git push
    $ git checkout my_new_local_branch_v2
...
Optionally you can to sign the commits using GPG signatures. Doing
