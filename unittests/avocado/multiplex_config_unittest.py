...
class CartesianConfigTest(unittest.TestCase):
    ...
    def testDefaults(self):
        ...
        self.assertRaises(multiplex_config.ParserError,
                          self._checkStringDump, """
    ...
    def testMissingInclude(self):
        self.assertRaises(multiplex_config.MissingIncludeError,
                          self._checkStringDump, """
    ...
    def testSyntaxErrors(self):
        self.assertRaises(multiplex_config.LexerError,
                          self._checkStringDump, """
                variants tests$:
                  - system1:
                        var = 1
    ...
    def testComplicatedFilter(self):
        ...
        lexer = multiplex_config.Lexer(multiplex_config.StrReader(f))
        lexer.set_prev_indent(-1)
        lexer.get_next_check([multiplex_config.LIndent])
        lexer.get_next_check([multiplex_config.LOnly])
        p_filter = multiplex_config.parse_filter(lexer, lexer.rest_line())
        self.assertEquals(p_filter,
                          [[[multiplex_config.Label("xxx"),
                             multiplex_config.Label("yyy")],
                            [multiplex_config.Label("xxx", "333"),
                             multiplex_config.Label("aaa")]],
                           [[multiplex_config.Label("ddd")]],
                           [[multiplex_config.Label("eeee")]],
                           [[multiplex_config.Label("rrr"),
                             multiplex_config.Label("aaa")]]],
