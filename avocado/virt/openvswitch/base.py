...
from avocado.versionable_class import VersionableClass, Manager, factory
...
class OpenVSwitchSystem(OpenVSwitchControlCli_CNT, OpenVSwitchControlDB_CNT):
    ...
    def check_db_daemon(self):
        ...
        """
        working = not process.is_alive("ovsdb-server", self.pid_files_path)
        if not working:
    ...
    def check_switch_daemon(self):
        ...
        Check if OVS daemon is started correctly.
        """
        working = process.is_alive("ovs-vswitchd", self.pid_files_path)
        if not working:
...
class OpenVSwitch(OpenVSwitchSystem):
    ...
    def init_new(self):
        ...
        self.db_pidfile = process.get_pid_path("ovsdb-server")
        self.ovs_pidfile = process.get_pid_path("ovs-vswitchd")
