...
from avocado.utils import passfd_setup
...
def create_monitor(vm, monitor_name, monitor_params):
    ...
    if monitor_params.get("monitor_type") == "qmp":
        ...
        if not cmdline.has_option("qmp", vm.qemu_binary):
...
class Monitor:
    ...
    def parse_info_numa(cls, r):
        ...
        data = [[0, set()] for _ in range(nodes)]
...
class HumanMonitor(Monitor):
    ...
    def block_mirror(self, device, target, speed, sync, fmt, mode,
...
class QMPMonitor(Monitor):
    ...
    def cmd(self, cmd, args=None, timeout=CMD_TIMEOUT, debug=True, fd=None):
        ...
        try:
            ...
            self._read_objects()
            # Send command
            q_id = crypto.get_random_string(8)
    ...
    def migrate_set_speed(self, value):
        ...
        :return: The response to the command
        """
        value = misc.convert_data_size(value, "M")
        args = {"value": value}
