"""
Classes and functions to handle block/disk images for QEMU.
...
class QemuImg(storage.base.QemuImg):

    """
    QEMU class for handling operations of disk/block images.
    """
    ...
    def __init__(self, params, root_dir, tag):
        ...
        """
        storage.base.QemuImg.__init__(self, params, root_dir, tag)
        self.image_cmd = path.get_qemu_img_binary(params)
        q_result = process.run(self.image_cmd, ignore_status=True,
                               verbose=False)
    ...
    def create(self, params, ignore_errors=False):
        ...
        if params.get("create_with_dd") == "yes" and self.image_format == "raw":
            ...
            if has_backing_file == "yes":
                ...
                backing_file = storage.base.get_image_filename(backing_param,
                                                               self.root_dir)
        ...
        if (params.get("image_backend", "filesystem") == "filesystem"):
            ...
            if image_dirname and not os.path.isdir(image_dirname):
                ...
                logging.error(e_msg)
                logging.error("This usually means a serious setup exception.")
                logging.error("Please verify if your data dir contains the "
                              "expected directory structure")
                logging.error("Data dir: %s",
                              data_dir.get_data_dir())
                logging.error("Directory structure:")
                for root, _, _ in os.walk(data_dir.get_data_dir()):
                    logging.error(root)
        ...
        cmd_result = process.run(qemu_img_cmd, verbose=False, ignore_status=True)
    ...
    def convert(self, params, root_dir, cache_mode=None):
        ...
        convert_image_filename = storage.base.get_image_filename(params_convert,
                                                                 root_dir)
    ...
    def check_image(self, params, root_dir):
        ...
        if (storage.base.file_exists(params, image_filename) or
            ...
            if not check_img:
                ...
                # so fail the test
                    ...
                    if chk == "yes":
                        self.backup_image(params, root_dir, "backup", False)
                    raise v_exceptions.VMImageCheckError(image_filename)
            ...
            if not storage.base.file_exists(params, image_filename):
...
class Iscsidev(storage.base.Iscsidev):
    ...
    def setup(self):
        ...
        if misc.wait_for(self.iscsidevice.get_device_name,
...
class LVMdev(storage.base.LVMdev):
