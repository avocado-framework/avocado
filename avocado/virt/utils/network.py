...
from avocado import propcan
from avocado.versionable_class import factory
...
class Macvtap(Interface):
    ...
    def __init__(self, tapname=None):
        if tapname is None:
            self.tapname = "macvtap" + crypto.get_random_id()
        else:
    ...
    def ip_link_ctl(self, ignore_status=False):
        return process.run(process.find_command("ip"), timeout=10,
...
class Bridge(object):
    ...
    def get_structure(self):
        ...
        out_line = (process.run(r"brctl show", verbose=False).stdout.splitlines())
        result = dict()
        bridge = None
        iface = None
...
def __init_openvswitch(func):
    ...
    def wrap_init(*args, **kargs):
        ...
        if __ovs is None:
            try:
                __ovs = factory(openvswitch_base.OpenVSwitchSystem)()
                __ovs.init_system()
                if (not __ovs.check()):
...
def get_net_if_operstate(ifname, runner=None):
    ...
    try:
        ...
        raise HwOperstarteGetError(ifname, "run operstate cmd exceptions.")
...
def change_iface_bridge(ifname, new_bridge, ovs=None):
    ...
    if type(ifname) is str:
        ...
        raise exceptions.TestSetupFail("Network interface %s is "
                                       "wrong type %s." % (ifname, new_bridge))
...
class DbNet(VMNet):
    ...
    def lock_db(self):
        if not hasattr(self, 'lock'):
            self.lock = io.lock_file(self.db_lockfile)
...
class VirtNet(DbNet, ParamsNet):
    ...
    def generate_ifname(self, nic_index_or_name):
        ...
        postfix = crypto.get_random_string(6)
...
def verify_ip_address_ownership(ip, macs, timeout=10.0):
    ...
    o = commands.getoutput("%s -f -c 3 -I %s %s" %
                           (process.find_command("arping"), dev, ip))
...
def restart_windows_guest_network_by_key(session, key, value, timeout=240,
    ...
    _ = get_windows_nic_attribute(session, key, value, oper_key, timeout)
    if not id:
