...
def glusterfs_mount(g_uri, mount_point):
    ...
    :type g_uri: str
    """
    disk.mount(g_uri, mount_point, "glusterfs", None,
               False)
...
def create_gluster_vol(params):
    ...
    if not hostname or hostname == "(none)":
        if_up = network.get_net_if(state="UP")
        for i in if_up:
            ipv4_value = network.get_net_if_addrs(i)["ipv4"]
...
def create_gluster_uri(params, stripped=False):
    ...
    if not gluster_server or gluster_server == "(none)":
        if_up = network.get_net_if(state="UP")
        ip_addr = network.get_net_if_addrs(if_up[0])["ipv4"][0]
...
def file_exists(params, filename_path):
    ...
    tmpdir = "gmount-%s" % (crypto.get_random_string(6))
    ...
    try:
        ...
        if disk.umount(sg_uri, tmpdir_path, False):
            try:
