...
from avocado.linux import fs

from avocado.virt import exceptions as v_exceptions
from avocado.virt.storage import gluster
from avocado.virt.storage import iscsi
from avocado.virt.storage import lvm
...
def get_image_blkdebug_filename(params, root_dir):
    ...
    if blkdebug_name is not None:
        blkdebug_filename = path.get_path(root_dir, blkdebug_name)
    else:
...
def get_image_filename_filesytem(params, root_dir):
    ...
    if indirect_image_select:
        ...
        try:
            ...
            raise v_exceptions.VMDeviceError("No matching disk found for "
        ...
        for protected in params.get('indirect_image_blacklist', '').split(' '):
            ...
            if match_image and match_image.group(0) == image_name:
                ...
                raise v_exceptions.VMDeviceError("Matching disk is in blacklist. "
                                                 "name = '%s', matching = '%s' and "
                                                 "selector = '%s'" %
                                                 (re_name, matching_images,
                                                  indirect_image_select))
    ...
    image_filename = path.get_path(root_dir, image_filename)
    ...
    def backup_image(self, params, root_dir, action, good=True,
        ...
        def get_backup_set(filename, backup_dir, action, good):
            ...
            if good:
                ...
                hsh = crypto.get_random_string(4)
        ...
        if params.get('image_raw_device') == 'yes':
            ...
            ifilename = path.get_path(root_dir, ifilename)
        ...
        if action == 'backup':
            ...
            image_dir_disk_free = fs.freespace(image_dir)
        ...
        if not params.get("image_name_%s_%s" % (image_name, vm_name)):
            ...
            if params.get("clone_master", "yes") == "yes":
                ...
                if not os.path.exists(image_fn) or force_clone == "yes":
                    ...
                    process.run(params.get("image_clone_command") % (m_image_fn,
                                                                     image_fn))
