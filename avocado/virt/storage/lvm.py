...
def normalize_data_size(size):
    if re.match(".*\d$", str(size)):
        size = "%s%s" % (size, UNIT)
    size = float(misc.normalize_data_size(size, UNIT, 1024))
...
class EmulatedLVM(LVM):
    ...
    def __init__(self, params, root_dir="/tmp"):
        process.find_command("losetup")
        process.find_command("dd")
    ...
    def get_emulate_image_name(self):
        ...
        return path.get_path(self.data_dir, img_path)
