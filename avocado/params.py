...
class AvocadoParams(object):
    ...
    provided list of leafs. Currently only first matching path contain the
    ...
    def __init__(self, leaves, test, test_id, tag):
        self.lock = Lock()
        self._params = []
        leaves = list(leaves)
        for i, path in enumerate(test.get_resolution_order()):
            path_leaves = self._get_params_from_leaves(path, leaves)
            self._params.append(AvocadoParam(path_leaves,
                                             '%d: %s' % (i, path)))
        self.id = test_id
        self.tag = tag
    ...
    def __str__(self):
        return "params {%s}" % ", ".join(_.name for _ in self._params)
    ...
    def _get_params_from_leaves(self, path, leaves):
        ...
        for leaf in [leaf for leaf in leaves if path.match(leaf.path + '/')]:
            # FIXME: This means used leafs are not reused. Is this expected
...
class AvocadoParam(object):
    ...
    """
    This is a single slice params. It can contain multiple leafs and tries to
    ...
    def __init__(self, leafs, name):
        # Basic initialization
        self._leafs = leafs
        self.name = name

    @property
    def _str_leafs_variant(self):
        leafs = [_.name for _ in self._leafs]
        return "%s (%s)" % (self.name, leafs)

    def _get_leaves(self, path):
        return [leaf for leaf in self._leafs if path.match(leaf.name)]
    ...
    def get_leaf(self, path):
        ...
        if len(leaves) == 1:
            ...
            raise NoMatchError('No leaves matchng "%s" pattern found in %s'
                               % (path.pattern, self._str_leafs_variant))
        else:
    ...
    def get_or_die(self, path, key):
        ...
        if len(ret) == 1:
            ...
            raise NoMatchError("No matches to %s =>%s in %s"
                               % (path, key, self._str_leafs_variant))
        else:
