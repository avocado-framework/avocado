...
class PeerInfo:
    ...
    def ping6_check(self, interface, count, option=None, flood=False):
        """
        IPV6 ping check to peer
        """
        cmd = "ip addr show %s" % interface
        try:
            for line in self.session.cmd(cmd).stdout.decode("utf-8") \
                                                    .splitlines():
                if "inet6" in line:
            ...
            if ipv6 == "":
                ...
                return False
        cmd = "ping -I %s %s -c %s" % (interface, ipv6, count)
        if flood is True:
            cmd = "%s -f" % cmd
        elif option is not None:
            cmd = "%s %s" % (cmd, option)
