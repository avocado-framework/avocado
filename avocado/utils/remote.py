...
from avocado.commander import master
from avocado.commander import messenger
from avocado.utils import io
from avocado.utils import rss_client
...
def remote_login(client, host, port, username, password, prompt, linesep="\n",
    ...
    try:
        ...
        raise
    if log_filename:
        session.set_output_func(io.log_line)
...
def remote_commander(client, host, port, username, password, prompt,
    ...
    cmd = master.CommanderMaster(inw, outw, False)
    return cmd
...
class RemoteRunner(object):
    ...
    def run(self, command, timeout=60, ignore_status=False):
        ...
        if (status and (not ignore_status)):
            raise exceptions.CmdError(command, cmd_result)
        return cmd_result
