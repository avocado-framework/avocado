...
def get_atomic_write_units(device):
    ...
    aw_min = (
        int(open(min_path, encoding="utf-8").read().strip())
        if os.path.exists(min_path)
        else 0
    )
    aw_max = (
        int(open(max_path, encoding="utf-8").read().strip())
    ...
    if devices is None:
        devs = (
            process.system_output(
                "ls /dev/nvme*n1 2>/dev/null", shell=True, ignore_status=True
            )
            .decode()
            .split()
        )
...
def create_partitions_in_free_space(device, count=2):
    ...
    before = set(
        process.system_output(
            f"lsblk -lnpo NAME {device} 2>/dev/null", shell=True, ignore_status=True
        )
        .decode()
        .split()
    )
    ...
    for i in range(count):
        ...
        process.run(
            f"parted -s {device} mkpart primary {p_start}MiB {p_end}MiB", sudo=True
        )
        cursor = p_end
...
def remove_partitions(device, partitions):
    for part in partitions:
        ...
        if m:
            process.run(
                f"parted -s {device} rm {m.group(1)}", sudo=True, ignore_status=True
            )
    process.run(f"partprobe {device}", sudo=True, ignore_status=True)
