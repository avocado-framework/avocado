...
def _find_partitions(devices, errors=None):
    ...
    for dev in devices:
        ...
        try:
            ...
            if errors is not None:
                errors.append(f"Cannot read partitions for {dev}: {e}")
            continue
    return parts


def _find_lvm_structures(  # pylint: disable=too-many-nested-blocks
    all_devs, logger, discovery_errors=None
):
    ...
    for dev in all_devs:
        ...
        elif result.stdout_text.strip():
            for line in result.stdout_text.splitlines():
                vg = line.strip()
                if vg:
                    vgs.add(vg)

    for vg in list(vgs):  # pylint: disable=too-many-nested-blocks
        try:
            if lv_utils.vg_check(vg):
                ...
                if result.exit_status:
                    if discovery_errors is not None:
                        discovery_errors.append(
                            f"pvs pv_name query failed for VG {vg} "
                            f"(exit {result.exit_status}): "
                            f"{result.stderr_text.strip()}"
                        )
                elif result.stdout_text.strip():
...
def _find_raid_arrays(  # pylint: disable=too-many-branches,too-many-locals,too-many-nested-blocks
    all_devs, errors=None
):
    ...
    for dev in all_devs:
        if os.path.exists(f"/dev/{dev}"):
            ...
            result = process.run(cmd, ignore_status=True, shell=True, sudo=True)
            if not result.exit_status and "Magic" in result.stdout_text:
    ...
    if os.path.exists("/proc/mdstat"):
        try:
            ...
            if errors is not None:
                errors.append(f"Cannot read /proc/mdstat: {e}")

    return raids


def _check_raid_for_lvm(  # pylint: disable=too-many-nested-blocks
    raids, existing_lvs, discovery_errors=None
):
    """Helper: Check RAID devices for LVM structures."""
    vgs = set()
    lvs = []

    for raid_dev in raids:  # pylint: disable=too-many-nested-blocks
        cmd = f"pvs --noheadings -o vg_name {raid_dev} 2>/dev/null"
        result = process.run(cmd, ignore_status=True, shell=True, sudo=True)
        if result.exit_status:
            if discovery_errors is not None:
                discovery_errors.append(
                    f"pvs query failed for RAID {raid_dev} "
                    f"(exit {result.exit_status}): "
                    f"{result.stderr_text.strip()}"
                )
            continue
        ...
        for line in result.stdout_text.splitlines():
            ...
            try:
                if lv_utils.vg_check(vg):
                    ...
                    if result2.exit_status:
                        if discovery_errors is not None:
                            discovery_errors.append(
                                f"lvs query failed for VG {vg} on RAID "
                                f"{raid_dev} (exit {result2.exit_status}): "
                                f"{result2.stderr_text.strip()}"
                            )
                    else:
                        new_lvs = [
                            (vg, lv.strip())
...
def _build_mount_points(devices, parts, raids, lvs):
    ...
    # _find_raid_arrays() returns absolute paths (/dev/md0, /dev/md/name).
    # Normalise to bare names so the generic alias loop below does not
    # produce double-slash paths like /dev//dev/md0.
    raid_names = [os.path.basename(r) for r in raids]
    all_devs = list(set(devices + parts + lv_devs + raid_names))

    mounts = []
    for dev in all_devs:
        mounts.extend([dev, f"/dev/{dev}", f"/dev/mapper/{dev}"])
    # Append the original absolute RAID paths directly — they are already
    # valid device paths and do not need the /dev/ prefix re-applied.
    for md in raids:
        mounts.append(md)
    for vg, lv in lvs:
...
def build_device_dependencies(devices, logger=None):
    ...
    :param devices: List of device names
    :param logger: Logger instance
    :return: Dict with 'parts', 'vgs', 'lvs', 'raids', 'devs', 'mounts',
        'pvs', 'discovery_errors'
    """
    log = logger or LOGGER
    discovery_errors = []

    parts = _find_partitions(devices, errors=discovery_errors)
    all_devs = devices + parts

    vgs, lvs, pvs = _find_lvm_structures(all_devs, log, discovery_errors)
    raids = _find_raid_arrays(all_devs, errors=discovery_errors)

    raid_vgs, raid_lvs = _check_raid_for_lvm(sorted(raids), lvs, discovery_errors)
    vgs.update(raid_vgs)
    lvs.extend(raid_lvs)
...
def unmount_devices(  # pylint: disable=too-many-branches,too-many-statements,too-many-locals,too-many-nested-blocks
    devices,
    logger=None,
    is_swap=False,
    max_retries=MAX_UNMOUNT_RETRIES,
    retry_delay=UNMOUNT_RETRY_DELAY_SECONDS,
):
    """
    ...
    for attempt in range(max_retries):
        ...
        for line in lines:
            ...
            for d in devices:
                ...
                if real_dev.startswith(real_d) and real_d:
                    ...
                    if remainder:
                        if real_d[-1].isdigit():
                            if remainder.startswith("p") and remainder[1:].isdigit():
                                match = True
                                break
        ...
        if not remaining:
            if not count and not attempt:
                log.info("  No %s found", "swap" if is_swap else "mounts")
            return errors

        if attempt < max_retries - 1:
            log.info(
                "  %d %s(s) failed, retrying in %ss (attempt %d/%d)",
                len(remaining),
                "swap" if is_swap else "mount",
                retry_delay,
                attempt + 1,
...
def remove_lvm_structures(  # pylint: disable=too-many-branches
    volume_groups, logical_volumes, logger=None
):
    """
    ...
    for vg in volume_groups:
        ...
        if not result.exit_status and result.stdout_text.strip():
            pvs = [
                p.strip() for p in result.stdout_text.strip().split("\n") if p.strip()
            ]
        ...
        if result.exit_status and "not found" not in result.stderr_text.lower():
...
def _clean_raid_members(members, logger):
    ...
    for m in members:
        if os.path.exists(m):
            ...
            if not result.exit_status and result.stdout_text.strip() == "LVM2_member":
...
def cleanup_raid_arrays(raid_devices, logger=None):
    ...
    for md in raid_devices:
        ...
        result = process.run(cmd, ignore_status=True, shell=True, sudo=True)
        if result.exit_status or not os.path.exists(mpath):
            continue
        ...
        if not result.exit_status and result.stdout_text.strip() == "LVM2_member":
            ...
            process.run(
                f"wipefs -af {mpath}", ignore_status=True, shell=True, sudo=True
            )
...
def wipe_disk_metadata(devices, logger=None):
    ...
    for dev in devices:
        ...
        # Handle device-mapper devices — resolve the real dm-N kernel name
        # so dmsetup suspend/resume targets the correct device node.
        # os.path.realpath resolves /dev/mapper/mpatha -> /dev/dm-0, etc.
        is_dm = "mapper" in dpath or dpath.startswith("/dev/dm-")
        if is_dm:
            dm_name = os.path.basename(os.path.realpath(dpath))
            process.run(
        ...
        for attempt in range(WIPE_RETRY_ATTEMPTS):
            ...
            if (
                not result1.exit_status
                or not result2.exit_status
                or "Device or resource busy" not in result2.stderr_text
            ):
                wiped = not result1.exit_status or not result2.exit_status
                if not wiped:
                    errors.append(f"wipefs failed on {dpath}: {result2.stderr_text}")
                break

            if attempt < WIPE_RETRY_ATTEMPTS - 1:
                log.debug(
                    "  Wipe retry %s/%s for %s",
                    attempt + 1,
                    WIPE_RETRY_ATTEMPTS,
                    dpath,
                )
            ...
            errors.append(
                f"Failed to wipe {dpath} after {WIPE_RETRY_ATTEMPTS} attempts "
...
def _remove_partition_tables(disks, logger=None):
    ...
    for dev in disks:
        dpath = get_absolute_disk_path(dev)
        log.debug("  Removing partition table from %s", dpath)
...
def _zero_disks(disks, logger, wipe_size_mb=DEFAULT_WIPE_SIZE_MB):
    ...
    :return: List of error messages for any failed ``dd`` operations.
    """
    errors = []
    for dev in disks:
        dpath = get_absolute_disk_path(dev)
        ...
        if result.exit_status:
            errors.append(
                f"dd zero failed on {dpath} (exit {result.exit_status}): "
                f"{result.stderr_text.strip()}"
            )
    return errors
...
def _settle_system(logger):
    ...
    process.run(
        f"udevadm settle --timeout={UDEV_SETTLE_TIMEOUT_SECONDS}",
        ignore_status=True,
        shell=True,
        sudo=True,
    )
    ...
    logger.info(
        "  Waited %ss for device stabilization", DEVICE_STABILIZATION_DELAY_SECONDS
    )


def cleanup_disks(  # pylint: disable=too-many-branches,too-many-statements,too-many-locals
    disk_list, logger=None, mode="light"
):
    """
    ...
    warnings.warn(
        "cleanup_disks() in avocado.utils.disk is deprecated and will be "
        "removed in a future release. Use autils.system.disk from the "
        "aautils package instead: "
        "https://github.com/avocado-framework/aautils",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    if non_str:
        raise ValueError(f"disk_list contains empty or non-string entries: {non_str!r}")
    ...
    invalid = [d for d in disk_list if not os.path.exists(get_absolute_disk_path(d))]
    if invalid:
        raise ValueError(
    ...
    log.info(
        "Found: %d parts, %d VGs, %d LVs, %d PVs, %d RAIDs",
        len(deps["parts"]),
        len(deps["vgs"]),
        len(deps["lvs"]),
        len(deps["pvs"]),
    ...
    if mode == "auto":
        discovery_errors = deps.get("discovery_errors", [])
        if discovery_errors:
            # Discovery incomplete — fall back to light mode to avoid
            # wiping a disk that may still have live structures we could
            # not enumerate.
            mode = "light"
            log.warning(
                "\n[AUTO-DETECT] Discovery errors — defaulting to light mode "
                "to avoid data loss"
            )
            for derr in discovery_errors:
                log.warning("  discovery: %s", derr)
        else:
    ...
    if mode == "full":
        ...
        log.info("\n[9/9] Zeroing disks")
        errors.extend(_zero_disks(disks, log, wipe_size_mb=100))

        _settle_system(log)
    else:
        log.info("\n[7/9] Skipping partition removal (light mode)")
        log.info("\n[8/9] Wiping metadata (dependent devices only, light mode)")
        # In light mode, wipe only the discovered dependent devices
        # (partitions, LVM PVs, RAID members) — not the raw parent disks.
        # Wiping the whole disk with wipefs + dd destroys the MBR/GPT header,
        # which contradicts the documented "light mode preserves partitions"
        # contract and also defeats auto-mode's reason for choosing light when
        # live structures were found.
        dep_devs = [
            d
            for d in deps["devs"]
            if d not in disks and not any(d == os.path.basename(p) for p in disks)
        ]
        if dep_devs:
            errors.extend(wipe_disk_metadata(dep_devs, log))
        else:
            log.info("  → No dependent devices to wipe")
