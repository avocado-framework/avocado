...
def fetch(params, search='asset'):
    ...
    asset:
        ...
        cache_dir: <path> # Optional. Default=/var/tmp.
        location: # list of locations to try.
        # Supported schemes are: file, http and ftp.
        - file://<path>
        - http://<url>
        - ftp://<url>

    :param params: yaml parsed options.
    :param search: (optional) search for yaml option with all the asset
            data. Default is 'asset'.
    :return: asset path.
    """

    name = params.get('name', '*/%s/*' % search, default=None)
    md5sum = params.get('md5sum', '*/%s/*' % search, default=None)
    cache_dir = params.get('cache_dir', '*/%s/*' % search, default='/var/tmp')
    location = params.get('location', '*/%s/*' % search, default=None)
    ...
    for item in location:
        urlobj = urlparse.urlparse(item)
        path = os.path.join(urlobj.path, name)
        url = urlparse.urlunparse(urlobj._replace(path=path))
        ...
        if urlobj.scheme == 'file':
            log.debug('Trying to copy from %s.' % path)
            try:
