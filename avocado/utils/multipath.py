...
def form_conf_mpath_file(blacklist="", defaults_extra=""):
    ...
    if blacklist:
        ...
        mpath_fp.write("}\n")
    mpath_fp.close()
    cmd = "cat /etc/multipath.conf"
    process.system(cmd, ignore_status=True)
    multipath_service(operation='restart')


def multipath_service(operation='restart'):
    """
    Start, Stop, Restart multipath service.

    :parm operation: multipath service operation to perform.
                     Can be one of start, stop, restart.

    :raise: :class:`MultipathError`, if ``multipath is inactive``.
    """
    if distro.detect().name == 'Ubuntu':
        service_name = "multipath-tools"
    else:
        service_name = "multipathd"
    cmd = "service %s %s" % (service_name, operation)
    time.sleep(5)
    process.system(cmd, ignore_status=True)
    if 'start' in operation and not multipath_isactive():
        cmd = "cat /etc/multipath.conf"
        process.system(cmd, ignore_status=True)
        raise MultipathError("Multipath service not active")


def multipath_isactive():
    """
    Checks if multipath service is active or not.

    :return: True if service is active, False if inactive.
    """
    if distro.detect().name == 'Ubuntu':
        service_name = "multipath-tools"
    else:
    ...
    out = process.system_output(cmd, ignore_status=True)
    for line in out.splitlines():
        if 'Active' in line:
            return line.split()[1] == 'active'
    return False
...
def get_mpath_name(wwid):
    ...
    return process.system_output(cmd, ignore_status=True).split()[0]
