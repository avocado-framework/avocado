...
def get_mpath_devices():
    ...
    for line in multipath_op:
        wwid_regex = re.compile("[0-9a-fA-F]{32}")
        if bool(wwid_regex.search(line)):
            ...
            multipath_dict[mpath_dev]["paths"][disk_att[1]]["hcil"] = \
                    disk_att[0]
            multipath_dict[mpath_dev]["paths"][disk_att[1]]["dm_st"] = \
                    disk_att[3]
            multipath_dict[mpath_dev]["paths"][disk_att[1]]["chk_st"] = \
                    disk_att[4]
            multipath_dict[mpath_dev]["paths"][disk_att[1]]["dev_st"] = \
                    disk_att[5]
            multipath_dict[mpath_dev]["paths"][disk_att[1]]["dev_t"] = \
                    disk_att[2]
