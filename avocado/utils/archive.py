...
def is_file_with_magic_bytes(path, magic_bytes):
    """
...
def is_gzip_file(path):
    ...
    """
    return is_file_with_magic_bytes(path, MAGIC_BYTES["gzip"]["magic"])
...
def bzip2_uncompress(path, output_path=None, force=False):
    """
    Extracts a bzip2 compressed file to the same directory.
    """
...
def is_lzma_file(path):
    ...
    if is_file_with_magic_bytes(path, MAGIC_BYTES["xz"]["magic"]):
        return True
...
def is_zstd_file(path):
    ...
    """
    return is_file_with_magic_bytes(path, MAGIC_BYTES["zstd"]["magic"])
...
class ArchiveFile:
    ...
    def _detect_archive_type(filename):
        ...
        # Check for TAR file
            ...
            if is_file_with_magic_bytes(filename, MAGIC_BYTES["gzip"]["magic"]):
