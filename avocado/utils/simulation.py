...
if __name__ == '__main__':
    ...
    # Test Bridge
    br = NetworkBridge("abc")
    br.clean()

    assert "abc" not in process.run("brctl show", ignore_status=True).stdout
    br = NetworkBridge("abc")
    time.sleep(2)
    assert "abc" in process.run("brctl show abc", ignore_status=True).stdout
    br.clean()
    time.sleep(2)
    assert "abc" not in process.run("brctl show", ignore_status=True).stdout
...
# Test dummy interfaces
    br = NetworkBridge("abc")
    sim = SimulationNetwork(bridge=br)
    sim.addiface("x")
    sim.addiface("y")
    assert "link" in sim.getValuesForDevice("x")
    assert "link" in sim.getValuesForDevice("y")
    sim.clean()
    assert "link" not in sim.getValuesForDevice("x")
    assert "link" not in sim.getValuesForDevice("y")
    br.clean()

# Test Virtual Ethernet interfaces
    br = NetworkBridge("abc")
    sim = SimulationNetworkVeth(bridge=br)
    sim.addiface("x")
    sim.addiface("y")
    assert "link" in sim.getValuesForDevice("x")
    assert "link" in sim.getValuesForDevice("y")
...
# Test TAP interfaces
    br = NetworkBridge("abc")
    sim = SimulationNetworkTap(bridge=br)
    sim.addiface("x")
    sim.addiface("y")
    assert "link" in sim.getValuesForDevice("x")
    assert "link" in sim.getValuesForDevice("y")
