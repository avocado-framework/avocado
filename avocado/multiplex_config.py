...
"""
Multiplex configuration file parser.

This file format allows you to specify a test matrix in a compact fashion.
...
terminals and non terminals are only for better reading of definitions.
...
::
    ...
    N = {S, DEL, FILTER, FILTER_NAME, FILTER_GROUP, PN_FILTER_GROUP, STAT, VARIANT, VAR-TYPE, VAR-NAME, VAR-NAME-F,
         VAR, COMMENT, TEXT, DEPS, DEPS-NAME-F, META-DATA, IDENTIFIER}``
...
_reserved_keys = {"name", "shortname", "dep"}
...
class ParserError(Exception):
    ...
    def __init__(self, msg, line=None, filename=None, linenum=None):
        """
        Error parsing a file or string.

        :param msg: Error message.
        :param line: Contents of line where the error happened.
        :param filename: File parsed.
...
class Filter(object):
    ...
    def __init__(self, lfilter):
        ...
        :param lfilter: :class:`avocado.multiplex_config.LFilter` instance.
        """
        self.filter = lfilter
...
class NoFilter(NoOnlyFilter):
    ...
    def requires_action(self, ctx, ctx_set, descendant_labels):
        """
        Verify if filter requires action.

        :param ctx: Filter context.
        :param ctx_set: Filter context set.
        :param descendant_labels: Descendant labels.
        :return: Boolean of whether filter requires action.
        """
        return self.match(ctx, ctx_set)
    ...
    def might_pass(self, failed_ctx, failed_ctx_set, ctx, ctx_set,
                   descendant_labels):
        """
        Verify if filter might pass.

        :param failed_ctx: Failed context.
        :param failed_ctx_set: Failed context set.
        :param ctx: Context.
        :param ctx_set: Context set.
        :param descendant_labels: Descendant labels
        :return: Boolean as to whether filter might pass.
        """
        for word in self.filter:
            for block in word:
                if (_match_adjacent(block, ctx, ctx_set) <
...
class LIdentifier(str):
    ...
    def check_char(self, chars):
        """
        Check if string contains given chars.

        :type self: object
        :param chars: List of chars.
        :return: :raise ParserError:
        """
        for t in self:
            if not (t in chars):
    ...
    def check_char_alpha(self, chars):
        """
        Check if string contain only chars
        """
        for t in self:
    ...
    def check_char_alpha_num(self, chars):
        """
        Check if string contain only chars
        """
        for t in self:
    ...
    def check_char_numeric(self, chars):
        """
        Check if string contain only chars
        """
        for t in self:
...
class Lexer(object):
    ...
    def match(self, line, pos):
        ...
        if self.fast and pos == 0:  # due to regexp
            cind = line[pos:].find(":")
        ...
        if self.rest_as_string:
            ...
            for pos, char in li:
                if char.isalnum() or char in spec_iden:    # alphanum+_-
                    chars += char
...
class Parser(object):
    ...
    def assign(self, key, value):
        ...
        :param key: String with the variant name.
    ...
    def _parse(self, lexer, node=None, prev_indent=-1):
        ...
        variants_allowed_in = [LLBracket, LColon, LIdentifier, LEndL]
            ...
            while True:
                ...
                if typet == LEndBlock:
                    if pre_dict:
                        ...
                        apply_predict(lexer, node, pre_dict)
                    return node
                ...
                if typet == LIdentifier:
                    ...
                    meta_in_expand_defaults = False
                    if var_name not in self.expand_defaults:
                        meta_in_expand_defaults = True
                    node4 = Node()
                    while True:
                    ...
                    while vtypet not in [LColon, LEndL]:
                        ...
                        tokens = lexer.get_next_check_nw(variants_allowed_in)
                        vtypet = type(tokens[-1])
    ...
    def get_dicts(self, node=None, ctx=[], content=[], shortname=[], dep=[]):
        ...
        def process_content(content, failed_filters):
            ...
            :param failed_filters: Failed filters.
            """
            blocked_filters = []
            for t in content:
