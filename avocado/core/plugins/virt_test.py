...
if 'VIRT_TEST_DIR' in os.environ:
    ...
    if virt_test_dir is None or not os.path.isdir(virt_test_dir):
        ...
        print('')
        print('[virt-test]')
        print('virt_test_dir=/valid/path/to/virt_test')
        print('')
...
class VirtTestOptionsProcess(object):
    ...
    def __init__(self, job):
        ...
        # common section
        self.options.vt_data_dir = settings.get_value('virt_test.common', 'data_dir', default=data_dir.get_data_dir())
            ...
            if self.options.vt_type:
                if self.options.url and self.options.dropin:
                    ...
                    if self.options.vt_type == 'libvirt':
                        ...
                        if self.options.vt_remove_guest_guest:
    ...
    def get_parser(self):
        if not self.options.show_job_log:
            standalone_test.handle_stdout(self.options)
        self._process_options()
        standalone_test.reset_logging()
        standalone_test.configure_console_logging(loglevel=logging.INFO)
...
class VirtTestCompatPlugin(plugin.Plugin):
    ...
    def configure(self, parser):
        ...
        vt_compat_group_debug = parser.runner.add_argument_group('Virt-Test compat layer - Debug options')
        ...
        vt_compat_group_common.add_argument("--vt-arch", action="store", dest="vt_arch",
                                            default=None,
                                            help=("Architecture under test. "
                                                  "If --vt-config is provided, this will be ignored. "
                                                  "Default: any that supports the given machine type"))
        vt_compat_group_common.add_argument("--vt-machine-type", action="store", dest="vt_machine_type",
                                            default=None,
        ...
        vt_compat_group_qemu.add_argument("--vt-qemu-dst-bin", action="store", dest="vt_dst_qemu_bin",
                                          ...
                                          help=("Path to a custom qemu binary to be tested for "
                                                ...
                                                "no attempt to set the qemu binaries will be made. "
                                                "Default path: %s" % qemu_bin_path))
        vt_compat_group_qemu.add_argument("--vt-accel", action="store", dest="vt_accel", default="kvm",
                                          help=("Accelerator used to run qemu (kvm or tcg). "
                                                "Default: kvm"))
        vt_compat_group_qemu.add_argument("--vt-nettype", action="store", dest="vt_nettype",
                                          default=nettype_default, help=qemu_nw_msg)
        vt_compat_group_qemu.add_argument("--vt-netdst", action="store", dest="vt_netdst",
                                          default="virbr0",
                                          help=("Bridge name to be used "
                                                "(if you chose bridge as nettype). "
                                                "Default: virbr0"))
        vt_compat_group_qemu.add_argument("--vt-vhost", action="store", dest="vt_vhost",
                                          default="off",
                                          help=("Whether to enable vhost for qemu "
                                                "(on/off/force). Depends on nettype=bridge. "
                                                "If --vt-config is provided, this will be ignored. "
                                                "Default: off"))
        vt_compat_group_qemu.add_argument("--vt-monitor", action="store", dest="vt_monitor",
                                          default='human',
                                          help="Monitor type (human or qmp). Default: human")
        vt_compat_group_qemu.add_argument("--vt-smp", action="store", dest="vt_smp",
                                          default='2',
                                          help=("Number of virtual cpus to use (1 or 2). "
                                                "If --vt-config is provided, this will be ignored. "
                                                "Default: 2"))
        vt_compat_group_qemu.add_argument("--vt-image-type", action="store", dest="vt_image_type",
                                          default="qcow2",
                                          help=("Image format type to use "
                                                "(any valid qemu format). "
                                                "If --vt-config is provided, this will be ignored. "
                                                "Default: qcow2"))
        vt_compat_group_qemu.add_argument("--vt-nic-model", action="store", dest="vt_nic_model",
                                          default="virtio_net",
                                          help=("Guest network card model. "
                                                "If --vt-config is provided, this will be ignored. "
                                                "(any valid qemu format). Default: virtio_net"))
        vt_compat_group_qemu.add_argument("--vt-disk-bus", action="store", dest="vt_disk_bus",
                                          default="virtio_blk",
                                          help=("Guest main image disk bus. One of " +
                                                " ".join(SUPPORTED_DISK_BUSES) +
                                                ". If --vt-config is provided, this will be ignored. "
                                                "Default: virtio_blk"))
        vt_compat_group_qemu.add_argument("--vt-qemu-sandbox", action="store", dest="vt_qemu_sandbox",
                                          default="on",
                                          help=("Enable qemu sandboxing "
                                                "(on/off). Default: on"))
        vt_compat_group_qemu.add_argument("--vt_qemu_defconfig", action="store", dest="vt_qemu_defconfig",
                                          default="yes",
                                          help=("Prevent qemu from loading sysconfdir/qemu.conf "
                                                "and sysconfdir/target-ARCH.conf at startup. "
                                                "(yes/no). Default: yes"))
        vt_compat_group_qemu.add_argument("--vt-malloc-perturb", action="store",
        ...
        vt_compat_group_libvirt.add_argument("--vt-install-guest", action="store_true",
                                             dest="vt_install_guest",
                                             help=("Install the guest using import method before "
                                                   "the tests are run."))
        vt_compat_group_libvirt.add_argument("--vt-remove-guest", action="store_true", dest="vt-remove_guest",
                                             help=("Remove the guest from libvirt. This will not "
                                                   "delete the guest's disk file."))
        vt_compat_group_debug.add_argument("--vt-no-cleanup", action="store_true",
                                           dest="vt_no_cleanup",
                                           default=False,
                                           help=("Don't clean up tmp files or VM processes at "
                                                 "the end of a virt-test execution. Default: False"))
