...
class TestSuite:
    def __init__(self, name, config, tests=None):
        self.name = name
        self.tests = tests or None  # this will force None when tests = []
        ...
        if config.get('run.dry_run.enabled'):
            self.convert_to_dry_run()
    ...
    def convert_to_dry_run(self):
        for i in range(self.size):
            self.tests[i] = [DryRunTest, self.tests[i][1]]

    @classmethod
    def from_config(cls, config, name=None, runner='nrunner'):
        ...
        if runner == 'nrunner':
            return cls.from_config_with_resolver(config, name)
        else:
            return cls.from_config_with_loader(config, name)

    @classmethod
    def from_config_with_loader(cls, config, name=None):
        references = config.get('run.references')
    ...
    @classmethod
    def from_config_with_resolver(cls, config, name=None):
        ignore_missing = config.get('run.ignore_missing_references')
        references = config.get('run.references')
