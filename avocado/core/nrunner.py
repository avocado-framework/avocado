...
class ExecRunner(BaseRunner):
    ...
    """
    def run(self):
        process = subprocess.Popen([self.runnable.uri,
                                    *self.runnable.args],
...
class TaskStatusService:
    ...
    async def open_connection(self):
        ...
        port = int(port)
        _, results = await asyncio.open_connection(host, port)
        return results

    async def post(self, status):
        if self.connection is None:
            self.connection = await self.open_connection()
    ...
    async def run(self):
        ...
        for status in runner.run():
            ...
            for status_service in self.status_services:
                await status_service.post(status)
            yield status
...
async def task_run(task, echo):
    async for status in task.run():
        echo(status)
