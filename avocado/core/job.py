...
class Job(object):
    ...
    def __init__(self, args=None, base_tmpdir=None):
        """
        Creates an instance of Job class.

        :param args: an instance of :class:`argparse.Namespace`.
        :param base_tmpdir: base location for job tmpdir, only used when
                            keep_tmp is off. In case it's base_tmpdir is None
                            new one will be created and cleaned after
                            execution, otherwise it's preserved.
        """
        if args is None:
        ...
        # Ignore the base_tmpdir in case keep_tmp is enabled
        if getattr(self.args, "keep_tmp", None) == "on":
            base_tmpdir = self.logdir
        elif base_tmpdir is None:
            # We are creating new base tmpdir, we have to remove it
    ...
    def __del__(self):
        """
        Make sure we cleanup after not-executed jobs.
        """
        self.cleanup()
