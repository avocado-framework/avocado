...
class LoaderUnhandledReferenceError(LoaderError):
    ...
    def __str__(self):
        ...
        return (
            f"Unable to resolve reference(s) '{ref1}' "
...
class TestLoaderProxy:
    ...
    def register_plugin(self, plugin):
        try:
            ...
            raise InvalidLoaderPlugin(
                f"Object {plugin} is not an instance " f"of TestLoader"
            )
    ...
    def load_plugins(self, config):
        ...
        def _str_loaders():
            ...
            for plugin in self.registered_plugins:
                out += f"  {plugin.name}: " f"{', '.join(_good_test_types(plugin))}\n"
            return out.rstrip("\n")
        ...
        if "@DEFAULT" in loaders:  # Replace @DEFAULT with unused loaders
            ...
            loaders = (
                loaders[:idx]
                + [plugin for plugin in supported_loaders if plugin not in loaders]
                + loaders[idx + 1 :]
            )
    ...
    def _update_mappings(self):
        ...
        self._decorator_mapping = {MissingTest: output.TERM_SUPPORT.fail_header_str}
        for plugin in self._initialized_plugins:
    ...
    def discover(self, references, which_tests=DiscoverMode.DEFAULT, force=None):
        ...
        def handle_exception(plugin, details):
            ...
            stacktrace.log_message(
                (f"Test discovery plugin {plugin} " f"failed: {details}"),
...
class TestLoader:
    ...
    def __init__(self, config, extra_params):  # pylint: disable=W0613
        if "allowed_test_types" in extra_params:
            ...
            if len(mapping) != 1:
                msg = (
                    f"Loader '{self.name}' supports multiple test types "
                ...
                raise LoaderError(
                    f"Loader '{self.name}' doesn't support "
        ...
        if extra_params:
            raise LoaderError(
                f"Loader '{self.name}' doesn't handle extra "
...
def add_loader_options(parser, section="run"):
    ...
    help_msg = (
        "Overrides the priority of the test loaders. You can specify "
...
class FileLoader(TestLoader):
    ...
    def discover(self, reference, which_tests=DiscoverMode.DEFAULT):
        ...
        if self.test_type:
            mapping = self.get_type_label_mapping()
            test_class = next(
                key for key, value in mapping.items() if value == self.test_type
            )
            for tst in tests:
    ...
    def _make_existing_file_tests(self, test_path, make_broken, subtests_filter):
        if test_path.endswith(".py"):
            return self._make_python_file_tests(test_path, make_broken, subtests_filter)
        else:
            return make_broken(NotATest, test_path, self.NOT_TEST_STR)

    def _make_nonexisting_file_tests(
        self, test_path, make_broken, subtests_filter, test_name
    ):
        ...
        if os.path.exists(test_path):
            ...
            return make_broken(
                NotATest,
                test_name,
                (f"File not found ('{test_name}'; " f"'{test_path}')"),
            )

    @staticmethod
    def _make_test(
        klass, uid, description=None, subtests_filter=None, **test_arguments
    ):
        """
    ...
    def _make_tests(self, test_path, list_non_tests, subtests_filter=None):
        ...
        if os.path.exists(test_path):
            if os.access(test_path, os.R_OK) is False:
                return make_broken(AccessDeniedPath, test_path, "Is not " "readable")
            return self._make_existing_file_tests(
                test_path, make_broken, subtests_filter
            )
        else:
            if os.path.islink(test_path):
                try:
                    if not os.path.isfile(os.readlink(test_path)):
                        return make_broken(
                            BrokenSymlink, test_path, "Is a " "broken symlink"
                        )
                except OSError:
                    return make_broken(
                        AccessDeniedPath, test_path, "Is not " "accessible."
                    )
            return self._make_nonexisting_file_tests(
                test_path, make_broken, subtests_filter, test_name
            )
