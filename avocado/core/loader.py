...
class FileLoader(TestLoader):
    ...
    def _find_avocado_tests(self, path, class_name=None, instrumented=False):
        """
        ...
        :type class: str
        :param instrumented: whether the test is (or inherits from) a
                             avocado.Test. Used to track that under recursion
        :type instrumented: boolean
        :returns: tuple where first item is dict with class name and additional
                  ...
                  force-disabled; the third item is a boolean telling if the
                  tests found are Avocado INSTRUMENTED tests.
        :rtype: tuple
        """
        ...
        for statement in mod.body:
            ...
            # Looking for a 'class Anything(anything):'
                ...
                for parent in parents[:]:
                    ...
                    ret = self._find_avocado_tests(path, parent_class,
                                                   instrumented)
                    res, dis, instrumented = ret
                    if res:
                        ...
                        for artifact in node.names:
                            ...
                            if artifact.name == reference:
                                ...
                                _, ppath, _ = imp.find_module(parent_module,
                                                              modules_paths)
                                ret = self._find_avocado_tests(ppath,
                                                               parent_class,
                                                               instrumented)
                                res, dis, instrumented = ret
                                if res:
        ...
        return result, disabled, instrumented
    ...
    def _make_existing_file_tests(self, test_path, make_broken,
        ...
        try:
            # Avocado tests
            tests, disabled, instrumented = self._find_avocado_tests(test_path)
            if instrumented and tests:
                test_factories = []
