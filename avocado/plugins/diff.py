...
class Diff(CLICmd):
    ...
    def configure(self, parser):
        ...
        parser.add_argument('--html', type=str,
                            ...
                            'result should be written. The value - (output to '
                            'stdout) is not supported since not all HTML '
                            'resources can be embedded into a single file '
                            '(page resources will be copied to the output '
                            'file dir)')
    ...
    def run(self, args):
        ...
        if getattr(args, 'html_output', None) is not None:
            try:
                ...
                job_diff_html = html_diff.make_file(job1_results,
                                                    job2_results,
                                                    fromdesc=job1_id,
                                                    todesc=job2_id)
                with open(args.html_output, 'w') as f:
                    f.writelines(job_diff_html)
    ...
    def _setup_job(self, job_id):
        if os.path.isdir(job_id):
            ...
            job_id = os.path.basename(os.path.normpath(resultsdir))[-7:]
