...
class Jobs(CLICmd):
    ...
    def _save_stream_to_file(self, stream, filename):
        ...
        if not os.path.isdir(dirname):
            print("Error: {} does not exist. Exiting...".format(dirname))
    ...
    def handle_output_files_command(self, config, jobs_results):
        ...
        try:
            ...
            print("Could not get job information: {}".format(ex))
            return exit_codes.AVOCADO_GENERIC_CRASH

        spawner = None
        spawner_name = config_data.get('nrun.spawner')
        if spawner_name == 'process':
            spawner = ProcessSpawner()
        elif spawner_name == 'podman':
            spawner = PodmanSpawner()

        if spawner is None:
            msg = ("Could not find the spawner for job {}. This command is "
        ...
        for test in results_data.get('tests'):
            test_id = test.get('id')
            print("Downloading files for test {} ...".format(test_id))
            try:
                for filename, stream in spawner.stream_output(job_id, test_id):
    ...
    def run(self, config):
        ...
        if subcommand == 'list':
            ...
            self.handle_output_files_command(config, results)
        return exit_codes.AVOCADO_ALL_OK
