...
class Jobs(CLICmd):
    ...
    def configure(self, parser):
        ...
        help_msg = "Download output files generated by tests on AVOCADO_TEST_OUTPUTDIR"
        output_files_parser = subcommands.add_parser("get-output-files", help=help_msg)
        settings.register_option(
            section="jobs.get.output_files",
            key="job_id",
            help_msg="JOB id",
            metavar="JOBID",
            default=None,
        ...
        for test in tests:
            ...
            try:
                ...
                for filename, stream in files_buffers:
                    dest = os.path.join(
                        destination, test_id.replace("/", "_"), filename
                    )
    ...
    def handle_output_files_command(self, config):
        ...
        if spawner is None:
            msg = (
                "Could not find the spawner for job %s. This command is "
        ...
        return self._download_tests(
            results_data.get("tests"), destination, job_id, spawner
        )
