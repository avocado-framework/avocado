...
class TAPResult(ResultEvents):
    ...
    def pre_tests(self, job):
        ...
        if self.config.get("job.run.result.tap.enabled"):
            log = open(
                os.path.join(job.logdir, "results.tap"), "w", 1, encoding="utf-8"
            )
            self.__open_files.append(log)
            self.__logs.append(file_log_factory(log))
    ...
    def end_test(self, result, state):
        ...
        if status == "PASS":
            ...
            self.__write(
                "ok %s %s  # Warnings were printed into warn log",
                result.tests_run,
                name,
            )
        elif status in ("SKIP", "CANCEL"):
            self.__write(
                "ok %s %s  # SKIP %s", result.tests_run, name, state.get("fail_reason")
            )
        else:
    ...
    def post_tests(self, job):
        if job.interrupted_reason is not None:
            for pending_test in range(
                job.result.tests_run + 1, job.result.tests_total + 1
            ):
...
class TAPInit(Init):
    ...
    def initialize(self):
        ...
        help_msg = (
            "Enable TAP result output and write it to FILE. Use "
        ...
        help_msg = (
            "Enables default TAP result in the job results directory. "
