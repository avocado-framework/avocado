...
class TAPResult(ResultEvents):
    ...
    def __init__(self, config):  # pylint: disable=W0613
        self.__logs = []
        self.__open_files = []
        self._future = config.get('_future')
        output = self._future.get('run.tap.output')
        if output == '-':
            log = LOG_UI.debug
        ...
        self.__include_logs = self._future.get('run.tap.include_logs')
    ...
    def pre_tests(self, job):
        ...
        if self._future.get('run.tap.job_result') == 'on':
