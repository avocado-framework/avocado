...
class TestRunner(plugin.Plugin):
    ...
    def configure(self, app_parser, cmd_parser):
        ...
        myparser.add_argument('-x', '--export', action='store_true', default=False,
                              help='Export test results (implies to archive it).')
        ...
        self.run_parser = myparser
        myparser.set_defaults(func=self.run_tests)
        self.configured = True

    def activate(self, app_args):
        if app_args.export is True:
            from avocado.plugins.jsonresult import JSONTestResult
            self.run_parser.set_defaults(archive=True)
            self.app_parser.set_defaults(json=True,
                                         test_result=JSONTestResult,
                                         json_output='*debuglog*')
