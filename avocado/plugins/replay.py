...
class Replay(CLI):
    ...
    def run(self, config):
        ...
        job_id = future.get('run.replay.job_id')
        if job_id is None:
            return

        err = None
        replay_ignore = future.get('run.replay.ignore')
        test_status = future.get('run.replay.test_status')
        ...
        # supplied.
        if future.get('run.replay.resume'):
            if not test_status:
